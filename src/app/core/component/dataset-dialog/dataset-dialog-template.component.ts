  ...
  this.onboardingService.GWASbuttonClicked(this.currentDataSet.Id,
    ...
    (error) => {
      this.toast.show(this.toast.simple({
        ...
        content: 'Insufficient data to execute GWAS',
...
getDatasets() {
  this.toast.show(
    this.toast.simple({
      position: 'toast-bottom-right',
      content: 'Please wait while Datasets are being loaded',
      hideDelay: 3000
    }));
            ...
            if (fileItem.file.name === processFileResult.data.FileName) {
                ...
                this.toast.show(
                  this.toast.simple({
                    ...
                    content: 'An error was encountered processing the ' + fileItem.file.fileTypeName + ' file. ' + fileItem.errorMessage,
                    hideDelay: 5000
                  }));
        }
      });
      ...
      this.uploader.queue.forEach( (fileItem, key) => {
        if (fileItem.file.name === processFileResult.data.FileName) {
            ...
            this.toast.show(
              this.toast.simple({
                    position: 'toast-bottom-left',
                    content: fileItem.file.fileTypeName + ' file processed successfully',
                    hideDelay: 5000
                }));
              }
    });
  ...
  this.uploader.queue.forEach( (fileItem, key) => {
    if (fileItem.file.name === error.data.FileName) {
        ...
        this.toast.show(
          this.toast.simple({
                position: 'toast-bottom-left',
                content: 'An Error occured while processing the file. ' + fileItem.errorMessage,
                toastClass: 'toast-error',
                hideDelay: 5000
            }));
...
this.toast.show(
  this.toast.simple({
        position: 'toast-bottom-left',
        content: 'Please select a Dataset to proceed.',
        toastClass: 'toast-error',
        hideDelay: 5000
    }));
return false;
...
this.toast.show(
    this.toast.simple({
        position: 'toast-bottom-left',
        content: 'Import will begin shortly. Please wait...',
        hideDelay: 10000
    }));
...
this.onboardingService.importFile(this.currentDataSet.Id).then( (result) => {
  if (result.data.Germplasm !== null) {
     ...
     this.toast.show(
         this.toast.simple({
              position: 'toast-bottom-left',
              content: 'File import Successful. Please wait while the files are getting uploaded.',
              hideDelay: 5000
          }));

      this.onboardingService.upload(this.currentDataSet.Id, params).then(function (uploadResult) {
          ...
          if (uploadResult.data.IsValid === false) {
             this.toast.show(
                 this.toast.simple({
                      position: 'toast-bottom-left',
                      content: 'Something went wrong while trying to upload files.' + uploadResult.data.Message,
                      toastClass: 'toast-error',
                      hideDelay: 5000
                  })
              );
